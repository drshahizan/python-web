...
In this assignment, we were ask to do some web scraping with the tool that we assigned on. We will try do do web scraping using scrapy on CompAsia.com. The data that we will be scraping are product's name, price, URL. After that, data will be put in a dataframe and export to a .CSV file. The csv file can be found with the name 'IphoneProduct.csv'.
